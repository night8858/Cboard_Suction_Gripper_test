 ...
 * ════════════════════════════════════════════════════════════════
 *
 *  帧通用结构: [AA] [CMD] [数据区...] [FF] [EE] [CRC8]
 *  CRC8 (poly=0x07, SMBUS, init=0x00), 计算范围包含 AA ~ EE（不含 CRC 自身）
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 01 : MCU → Host  状态反馈帧（53 字节，建议 20 Hz）
 * ────────────────────────────────────────────────────────────────
 *  [0] AA      帧头
 *  [1] 01      命令字
 *  [2]..[5]    LFx      float (4B LE)  左前臂末端 X
 *  [6]..[9]    LFy      float (4B LE)  左前臂末端 Y
 *  [10]..[13]  RFx      float (4B LE)  右前臂末端 X
 *  [14]..[17]  RFy      float (4B LE)  右前臂末端 Y
 *  [18]..[21]  LBx      float (4B LE)  左后臂末端 X
 *  [22]..[25]  LBy      float (4B LE)  左后臂末端 Y
 *  [26]..[29]  RBx      float (4B LE)  右后臂末端 X
 *  [30]..[33]  RBy      float (4B LE)  右后臂末端 Y
 *  [34]..[37]  YAW      float (4B LE)  云台偏航角 (°)
 *  [38]..[41]  PITCH    float (4B LE)  云台俯仰角 (°)
 *  [42]        valve0   uint8_t        电磁阀 0 状态 (0/1)
 *  [43]        valve1   uint8_t        电磁阀 1 状态 (0/1)
 *  [44]        valve2   uint8_t        电磁阀 2 状态 (0/1)
 *  [45]        valve3   uint8_t        电磁阀 3 状态 (0/1)
 *  [46]        sw0      uint8_t        微动开关 0（预留）
 *  [47]        sw1      uint8_t        微动开关 1（预留）
 *  [48]        sw2      uint8_t        微动开关 2（预留）
 *  [49]        sw3      uint8_t        微动开关 3（预留）
 *  [50]        FF      帧尾
 *  [51]        EE      帧尾
 *  [52]        CRC8
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 02 : Host → MCU  机械臂控制帧（14 字节）
 * ────────────────────────────────────────────────────────────────
 *  [0] AA      帧头
 *  [1] 02      命令字
 *  [2]         armID   uint8_t        机械臂 ID (0=LF, 1=RF, 2=LB, 3=RB)
 *  [3]..[6]    x       float (4B LE)  末端目标 X (mm)
 *  [7]..[10]   y       float (4B LE)  末端目标 Y (mm)
 *  [11]        FF     帧尾
 *  [12]        EE     帧尾
 *  [13]        CRC8
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 03 : Host → MCU  云台控制帧（14 字节）
 * ────────────────────────────────────────────────────────────────
 *  [0] AA      帧头
 *  [1] 03      命令字
 *  [2]         gimbalID  uint8_t      云台 ID（当前仅支持 0）
 *  [3]..[6]    YAW       float (4B LE) 目标偏航角 (°)
 *  [7]..[10]   PITCH     float (4B LE) 目标俯仰角 (°)
 *  [11]        FF     帧尾
 *  [12]        EE     帧尾
 *  [13]        CRC8
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 04 : Host → MCU  电磁阀控制帧（7 字节）
 * ────────────────────────────────────────────────────────────────
 *  例: AA 04 01 01 → 电磁阀 1 打开
 *       AA 04 01 00 → 电磁阀 1 关闭
 *  [0] AA      帧头
 *  [1] 04      命令字
 *  [2]         valveID   uint8_t      电磁阀 ID (0~3)
 *  [3]         state     uint8_t      状态 (1=开, 0=关)
 *  [4]         FF      帧尾
 *  [5]         EE      帧尾
 *  [6]         CRC8
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 05 : Host → MCU  语音模块控制帧（8 字节）
 * ────────────────────────────────────────────────────────────────
 *  收到后通过 IIC/串口控制语音模块播放对应语音（当前占位）
 *  [0] AA      帧头
 *  [1] 05      命令字
 *  [2]         answer    uint8_t      答案编号
 *  [3]         00        uint8_t      预留
 *  [4]         00        uint8_t      预留
 *  [5]         FF       帧尾
 *  [6]         EE       帧尾
 ...
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 06 : Host → MCU  气泵控制帧（10 字节）
 * ────────────────────────────────────────────────────────────────
 *  [0] AA      帧头
 *  [1] 06      命令字
 *  [2]         on_off    uint8_t      开关 (1=启动, 0=停止)
 *  [3]..[6]    speed     float (4B LE) 目标转速 (RPM)
 *  [7]         FF       帧尾
 *  [8]         EE       帧尾
 *  [9]         CRC8
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 07 : Host → MCU  全量控制帧（58 字节，预留）
 * ────────────────────────────────────────────────────────────────
 *  单帧下发所有执行器目标值（机械臂、云台、阀门、电磁铁、气泵）
 *  [0] AA      帧头
 *  [1] 07      命令字
 *  [2]..[33]   四臂末端坐标  LFx LFy RFx RFy LBx LBy RBx RBy
 *                           各 float (4B LE), 共 32 B
 *  [34]..[41]  YAW PITCH   各 float (4B LE), 共 8 B
 *  [42]..[45]  valve0~3    uint8_t ×4  电磁阀状态
 *  [46]..[49]  emag0~3     uint8_t ×4  电磁铁状态
 *  [50]        pump_state  uint8_t     气泵开关
 *  [51]..[54]  pump_speed  float (4B LE) 气泵转速
 *  [55]        FF      帧尾
 *  [56]        EE      帧尾
 *  [57]        CRC8
 *
 * ────────────────────────────────────────────────────────────────
 *  AA 08 : Host → MCU  动作控制帧（9 字节）
 * ────────────────────────────────────────────────────────────────
 *  trigger=1 → 触发物料交接动作
 *  trigger=0 → 中止物料交接动作
 *  [0] AA      帧头
 *  [1] 08      命令字
 *  [2]         pair_idx  uint8_t      动作对编号 (0~1)
 *  [3]         dir_id    uint8_t      方向 ID (0/1)
 *  [4]         trigger   uint8_t      触发 (1=执行, 0=中止)
 *  [5]         00        uint8_t      预留
 *  [6]         FF       帧尾
 *  [7]         EE       帧尾
 *  [8]         CRC8
...
void cmd_execute_all(const all_pc_command *cmd)
{
    ...
    /* ── 气泵控制（同步 PumpCtrl 状态 + 直接输出） ── */
    {
        extern PumpCtrl g_pump;
        g_pump.target_speed_rpm = cmd->pump_target_speed;
        g_pump.is_running = (cmd->pump_target_speed > 1.0f);
